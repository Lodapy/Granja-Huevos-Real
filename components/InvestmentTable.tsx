    ...
    <div className="overflow-hidden rounded-xl border border-border bg-black/20">
      <table className="w-full text-sm text-left border-collapse">
        <thead className="bg-secondary/20 text-accent uppercase text-xs font-semibold tracking-wider">
          <tr>
            <th className="px-4 py-3 pl-10 border-b border-border">{headers[0]}</th>
          ...
          {items.map((item, index) => {
            ...
            return (
              <React.Fragment key={index}>
                ...
                >
                  <td className="px-4 py-3 text-slate-200 font-medium relative">
                    {hasDetails && (
                      <div className="absolute left-3 top-1/2 -translate-y-1/2 text-accent/70">
                        {isExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                      </div>
                    )}
                    <div className="pl-6">{item.category}</div>
                  </td>
                  <td className="px-4 py-3 text-right text-white tabular-nums font-serif tracking-wide">
                    {new Intl.NumberFormat('es-PY').format(item.amount)}
                  </td>
                  {exchangeRate && (
                    <td className="px-4 py-3 text-right text-emerald-400 tabular-nums font-serif tracking-wide opacity-90">
                      {formatUSD(usdValue)}
                    </td>
                  )}
                </tr>
                {isExpanded && hasDetails && (
                  <tr className="bg-black/40 shadow-inner">
                    <td colSpan={exchangeRate ? 3 : 2} className="px-4 py-3 pl-10">
                      <div className="grid gap-2 border-l border-accent/30 pl-4 py-1">
                        {item.details?.map((detail, dIdx) => (
                          <div key={dIdx} className="flex justify-between items-center text-xs text-slate-400">
