    ...
    <div className="rounded-xl border border-border bg-black/20 overflow-hidden w-full">
      <div className="w-full">
        <table className="w-full text-sm sm:text-base text-left border-collapse">
          <thead className="bg-secondary/20 text-accent uppercase text-xs sm:text-sm font-semibold tracking-wider">
            <tr>
            ...
            {items.map((item, index) => {
              ...
              return (
                <React.Fragment key={index}>
                  ...
                  >
                    <td className="px-2 sm:px-4 py-3 text-slate-200 font-medium relative text-sm sm:text-base">
                    ...
                    <td className="px-2 sm:px-4 py-3 text-right text-white tabular-nums font-serif tracking-wide whitespace-nowrap text-base sm:text-lg font-semibold">
                      {new Intl.NumberFormat('es-PY').format(item.amount)}
                    </td>
                    {exchangeRate && (
                      <td className="px-2 sm:px-4 py-3 text-right text-emerald-400 tabular-nums font-serif tracking-wide opacity-90 whitespace-nowrap text-base sm:text-lg font-semibold">
                        {formatUSD(usdValue)}
                      </td>
                    )}
                  </tr>
                      ...
                      <td colSpan={exchangeRate ? 3 : 2} className="px-4 py-3">
                        <div className="grid gap-2 border-l border-accent/30 pl-4 py-1">
                          {item.details?.map((detail, dIdx) => (
                            <div key={dIdx} className="flex flex-col sm:flex-row sm:justify-between sm:items-center text-xs sm:text-sm text-slate-400 gap-1 sm:gap-0">
